...
def pdRename (pd, oldName, newName):
    """Renames a chosen column to a given name"""
    pd = pd.rename(columns = {oldName: newName}, inplace = False)
    return(pd)
...
def newCases(df):
    ...
    sortedList = pdRename(sortedList, 'Total', 'New Cases')
...
tableDateFixed = pdRename(table, 'Total', date) #Renames the cmbined cases column frot 'Total' to the correct date
